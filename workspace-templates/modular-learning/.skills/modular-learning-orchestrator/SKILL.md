...
These tools are provided by Inno Agent, not implemented by this skill. If a tool is unavailable,
memory is disabled, or a query returns no useful evidence, explain the limitation briefly and
continue from the current conversation and learner-confirmed information. Never fabricate tool
calls, a learner profile, sources, or successful memory writes. The entry modes and knowledge
evidence labels below are reasoning aids, not extra runtime fields or tool parameters.
...
Inspect the archive result before describing a memory update. Only mention new Wiki pages or
relationships when the result confirms they were written; match the notice to the actual result.
Queued, disabled, duplicate, unchanged, partial, or failed results are not proof of a graph update.
Do not promise automatic graph expansion on every question.
